...
from high_frequency_final_test import patch_decoder_resblocks_h_and_cnt_hf, make_content_injection_schedule
...
def main():
    ...
    # 01/16 예령 수정
    # inj 시작, 끝 스텝 args 추가
    parser.add_argument("--inj_start", default=0, type=int, help="high frequency injection start step index")
    parser.add_argument("--inj_end", default=50, type=int, help="high frequency injection end step index")
    ...
    dataset = TripleImageDataset(opt.data_root, "yr_dataset.txt", image_size=opt.H, device=device)
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            schedule = make_content_injection_schedule(
                sampler.ddim_timesteps, 
                start_idx=opt.inj_start, 
                end_idx=opt.inj_end
            )
            print("Content injection schedule timesteps:", schedule)
