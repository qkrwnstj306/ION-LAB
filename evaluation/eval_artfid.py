...
def compute_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    # 생성 이미지, 실제 이미지의 통계를 이용해 FID 계산 (작을수록 두 분포가 가까움)
    ...
    covmean, _ = linalg.sqrtm(sigma1.dot(sigma2), disp=False)  #행렬곱의 행렬제곱근 
    if not np.isfinite(covmean).all(): # covmean에 NaN/Inf가 있다면,
...
def compute_activation_statistics(files, model, batch_size=50, device='cpu', num_workers=1):
    ...
    mu = np.mean(act, axis=0)  #평균
    sigma = np.cov(act, rowvar=False)  #공분산
    return mu, sigma
...
def compute_fid(path_to_stylized, path_to_style, batch_size, device, num_workers=1):
    ...
    mu1, sigma1 = compute_activation_statistics(stylized_image_paths, model, batch_size, device, num_workers) #stylized image 전체 집합
    mu2, sigma2 = compute_activation_statistics(style_image_paths, model, batch_size, device, num_workers) #style 이미지 전체 집합 
...
def compute_fid_infinity(path_to_stylized, path_to_style, batch_size, device, num_points=15, num_workers=1):
    # FID는 샘플 개수가 커질수록 안정적이지만 샘플 수가 적으면 편향이 생김 > FID의 샘플 수 무한대 극한값 추정해서 사용 
    # 샘플 수가 충분히 많았다면 얻을 수 있는 FID 예측
    ...
    # 결과 shape : (num_images, feature_dim)
    ...
    fids = []
    # 샘플 개수를 5000 ~ 전체 이미지 개수까지 설정하고 그때마다 무작위 subset을 뽑아 FID 계산
    ...
    # 직선 외삽으로 FID 추정 
    # 경험적으로 FID(N)은 1/N에 대해 거의 선형적으로 감소 -> (1/N, FID(N))의 데이터를 선형회귀
    # 1/N -> 0, (N>무한대) 일 때의 절편 = FID 무한대의 값
...
def compute_content_distance(path_to_stylized, path_to_content, batch_size, content_metric='lpips', device='cuda', num_workers=1, gray=False):
    ...
    # stylized, content를 같은 순서로 DataLoader에 넣고 배치 단위로 pair를 뽑을 수 있도록 준비
    ...
    elif content_metric == 'lpips': # 인간의 지각과 유사
    ...
    # 각 배치마다 콘텐츠 유사도 계산, 전체 평균을 내서 최종 콘텐츠 거리 반환
    # stylized, content 1:1 매칭 시켜 평균 유사도 측정 (낮을수록 좋음)
    dist_sum = 0.0
    N = 0
...
def compute_patch_simi(path_to_stylized, path_to_content, batch_size, device, num_workers=1):
    # stylized, content 사이 패치 단위 유사도 계산 
    """Computes the distance for the given paths.
    ...
    # Sort paths in order to match up the stylized images with the corresponding content image
    # 같은 인덱스의 stylized image, content image가 짝을 이루도록 정렬
    stylized_image_paths = get_image_paths(path_to_stylized, sort=True)
    content_image_paths = get_image_paths(path_to_content, sort=True)
    ...
    return dist_sum / N # 전체 이미지 쌍의 평균 거리 (값이 작을수록 보존이 잘 됨)
...
def compute_art_fid(path_to_stylized, path_to_style, path_to_content, batch_size, device, mode='art_fid_inf', content_metric='lpips', num_workers=1):
    # ArtFID = (content distance + 1) x (FID + 1)
    """Computes the FID for the given paths.
    ...
    art_fid_value = (cnt_value + 1) * (fid_value + 1)  #+1은 아주 작은 값이 곱해지지 않도록 안정화 
    ...
    return art_fid_value.item(), fid_value.item(), cnt_value.item(), gray_cnt_value.item(), # 낮을수록 좋음
