...
def compute_fid_infinity(path_to_stylized, path_to_style, mask_path, batch_size, device, num_points=15, num_workers=1):#mask넣어주기
...
#######char와 back일때 다르게 인자 주기
    # activations_stylized = get_activations_w_mask(stylized_image_paths, model, mask_image_paths, batch_size, device, num_workers)#stylized에만 mask 넣어주기, char
    activations_stylized = get_activations_w_mask(stylized_image_paths, model, mask_image_paths, batch_size, device, num_workers, back=True)#stylized에만 mask 넣어주기, back
#######
...
def compute_art_fid(path_to_stylized, path_to_style, path_to_content, mask_path, batch_size, device, mode='art_fid_inf', content_metric='lpips', num_workers=1):#mask넣어주기
    """Computes the FID for the given paths.
    ...
    if mode == 'art_fid_inf':
        fid_value = compute_fid_infinity(path_to_stylized, path_to_style, mask_path, batch_size, device, num_workers)#mask넣어주기
    elif mode == 'art_fid':
